...
const configSchema = Joi.alternatives()
...
exports.createBoxSchema = Joi.object({
  ...baseSchema,
  name: baseSchema.name.required(),
  price: baseSchema.price.required(),
}).messages({
  "any.required": "Missing required box payload field",
});

exports.updateBoxSchema = Joi.object({
  ...baseSchema,
}).min(1).messages({
